...
impl Fees {
    ...
    pub fn fee_pct(trade_fee_rate: u64) -> Option<Decimal> {
        let trade_fee_pct = if trade_fee_rate > 0 {
            Decimal::from_u64(trade_fee_rate)?
                .checked_div(Decimal::from_u64(FEE_RATE_DENOMINATOR_VALUE)?)?
        } else {
            Decimal::ZERO
        };
